...
mutable struct MultivariateKalmanState{Fl <: AbstractFloat}
    ...
    TPtt::Matrix{Fl}
    function MultivariateKalmanState(num_series::Int, a1::Vector{Fl}, P1::Matrix{Fl}) where Fl
        m = length(a1)
...
function save_a1_P1_in_filter_output!(filter_output::FilterOutput{Fl},
                                      kalman_state::MultivariateKalmanState{Fl}) where Fl
...
mutable struct MultivariateKalmanFilter{Fl <: Real} <: KalmanFilter
    ...
    function MultivariateKalmanFilter(num_series::Int,
                                    a1::Vector{Fl},
                                    P1::Matrix{Fl},
...
function update_v!(kalman_state::MultivariateKalmanState{Fl}, y::Vector{Fl}, 
                   Z::Matrix{Fl}, d::Vector{Fl}) where Fl
    kalman_state.v .= y .- Z * kalman_state.a .- d
    return
end
function update_F!(kalman_state::MultivariateKalmanState{Fl}, Z::Matrix{Fl}, H::Matrix{Fl}) where Fl
    kalman_state.F = Z * kalman_state.P * Z' .+ H
    return
end
function update_att!(kalman_state::MultivariateKalmanState{Fl}, Z::Matrix{Fl}) where Fl
    kalman_state.att = kalman_state.a + kalman_state.P * Z' * inv(kalman_state.F) * kalman_state.v
    return
end
...
function update_a!(kalman_state::MultivariateKalmanState{Fl}, T::Matrix{Fl}, c::Vector{Fl}) where Fl
...
function update_Ptt!(kalman_state::MultivariateKalmanState{Fl}, Z::Matrix{Fl}) where Fl
    kalman_state.Ptt = kalman_state.P - 
                (kalman_state.P * Z' * inv(kalman_state.F) * Z * kalman_state.P)
    return
end
...
function update_P!(kalman_state::MultivariateKalmanState{Fl}, T::Matrix{Fl},
...
function update_llk!(kalman_state::MultivariateKalmanState{Fl}) where Fl
    kalman_state.llk -= (HALF_LOG_2_PI + 0.5*(logdet(kalman_state.F) + 
                                             kalman_state.v' * inv(kalman_state.F) * kalman_state.v))
    return
end

function update_kalman_state!(kalman_state::MultivariateKalmanState{Fl}, y::Vector{Fl}, Z::Matrix{Fl}, T::Matrix{Fl},
                              H::Matrix{Fl}, RQR::Matrix{Fl}, d::Vector{Fl}, c::Vector{Fl}, skip_llk_instants::Int,
...
function update_kalman_state!(kalman_state::MultivariateKalmanState{Fl}, y::Vector{Fl}, Z::Matrix{Fl}, T::Matrix{Fl},
                              H::Matrix{Fl}, R::Matrix{Fl}, Q::Matrix{Fl}, d::Vector{Fl}, c::Vector{Fl},
...
function optim_kalman_filter(sys::StateSpaceSystem,
                             filter::MultivariateKalmanFilter{Fl}) where Fl
...
function filter_recursions!(kalman_state::MultivariateKalmanState{Fl},

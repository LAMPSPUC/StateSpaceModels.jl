...
CRIT_VALS_05 = [0.470, 0.749, 1.01, 1.24, 1.47, 1.68, 1.9, 2.11, 2.32, 2.54, 2.75, 2.96]

function crit_val_05_generalized_von_mises_distribution(s)
    return s > 12 ? 0.269*s*0.928 : CRIT_VALS_05[s]
end

function π_statistic(ε::Vector{Fl}, σ::Float64, T::Int64) where Fl
    return (T^(-2)*σ^(-2))*
            sum(
...
function std_statistic(ε::Vector{Fl}, σ::Float64, T::Int64, s::Int64, j::Int64) where Fl
    return (2*T^(-2)*σ^(-2))*
            sum(
...
function test_statistic(ε::Vector{Fl}, σ::Float64, T::Int64, s::Int64) where Fl
    ω = Float64[]
...
function build_std(T::Int64, s::Int64, j::Int64)
    t = collect(1:T)
    return [cos.(λ(j, s).*t) sin.(λ(j, s).*t)]
end

function build_π(T::Int64, s::Int64, j::Int64)
    t = collect(1:T)
    return [cos.(λ(j, s)*t) ones(T)]
end

function build_X(s::Int64, T::Int64)
...
function seasonal_regression(y::Vector{Fl}, s::Int64, T::Int64) where Fl
    X = build_X(s, T)

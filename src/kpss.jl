...
function σ²_estimator(l::Int64, n::Int64, ϵ::Vector{Float64})
...
function KPSSTest_trend(y::Vector{Float64}, lags::Bool)
...
function KPSSTest_level(y::Vector{Float64}, lags::Bool)
...
function calc_η_trend(y::Vector{Float64}, lags::Bool)
    n     = length(y)
    zt    = collect(1:n)
    r     = ones(n)
    x     = hcat(r,zt)
...
function calc_η_level(y::Vector{Float64}, lags::Bool)
    n     = length(y)
    ϵ     = y .- (mean(y))
    l     = calc_lag_kpss(lags, n)
    η     = (1/n^2)*sum(sum((((sum(ϵ[i] for i in 1:t))^2)/σ²_estimator(l, n, ϵ)) for t in 1:n))
...
function KPSSTest(y::Vector{Float64}; null::Symbol = :level, lags::Bool = true)

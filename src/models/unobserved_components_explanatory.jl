...
@doc raw"""
UnobservedComponentsExplanatory(
        y::Vector{Fl}, X::Matrix{Fl}; 
...
"""
mutable struct UnobservedComponentsExplanatory <: StateSpaceModel
    ...
    function UnobservedComponentsExplanatory(y::Vector{Fl}, X::Matrix{Fl}; 
                                  trend::String = "local level",
        ...
        if cycle != "no"
            @warn "UnobservedComponentsExplanatory is currently unstable when cycle is included."
        end
        ...
        T = build_T(Fl, num_observations, num_exogenous, has_trend, has_slope, has_seasonal, seasonal_freq, has_cycle)
        R = build_R(Fl, num_observations, num_exogenous, has_trend, has_slope, has_seasonal, seasonal_freq, has_cycle,
        ...
        names = build_names(num_exogenous, has_irregular, stochastic_trend, stochastic_slope, 
...
has_exogenous(::UnobservedComponentsExplanatory) = true

num_components(model::UnobservedComponentsExplanatory) = (model.has_trend + model.has_slope + model.has_seasonal +
                                                          model.has_cycle + size(model.exogenous, 2))
function dict_components(model::UnobservedComponentsExplanatory)
...
function build_names(num_exogenous::Int,
                     has_irregular::Bool,
                     stochastic_trend::Bool, 
                     stochastic_slope::Bool, 
                     stochastic_seasonal::Bool,
...
function sigma2_names(model::UnobservedComponentsExplanatory)
...
# Obligatory methods
function default_filter(model::UnobservedComponentsExplanatory)
...
function initial_hyperparameters!(model::UnobservedComponentsExplanatory)
    Fl = typeof_model_elements(model)
    ...
    if model.has_cycle
        ...
        if model.damped_cycle
            initial_hyperparameters["ρ _cycle"] = Fl(0.7)
        end
    end
...
function get_beta_name(model::UnobservedComponentsExplanatory, i::Int)
...
function constrain_hyperparameters!(model::UnobservedComponentsExplanatory)
    for i in axes(model.exogenous, 2)
...
function unconstrain_hyperparameters!(model::UnobservedComponentsExplanatory)
    for i in axes(model.exogenous, 2)
...
function fill_model_system!(model::UnobservedComponentsExplanatory)
...
function fill_H_in_time(model::UnobservedComponentsExplanatory, H::Fl) where Fl
    return fill_system_matrice_with_value_in_time(model.system.H, H)
end

function reinstantiate(model::UnobservedComponentsExplanatory, y::Vector{Fl}, X::Matrix{Fl}) where Fl
    return UnobservedComponentsExplanatory(y, X; 
                                trend = model.trend, 
